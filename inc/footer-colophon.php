...
function mm4_footer_colophon() {
    ...
    <div class="footer-address-section">
        <div class="footer-address">
            ...
            <span>Email: <a href="mailto:<?php echo $email; ?>"><?php echo $email; ?></span>
        </div>
    </div>
    ...
    <div class="wrapper">
        <div class='stfcs-score-box' style='margin:0 auto; width: 180px; border: 1px solid #666; font: normal 10px arial; text-align:center;padding:10px 10px 0;'>
        <a href='http://www.satisfacts.com' target='_blank' style='text-decoration: none; color: black'>
          <img style="width:100%; height: auto;" src='http://ssnip.satisfacts.com/WebServices/Images/SFR-logo-300x64.jpg' />
          <p style='margin-bottom:5px; font-size: 9px'>Average Property Score: <strong id='stfcs-score-number'>N/A</strong></p>
          <p style='margin:0; font-size: 9px; color: gray'>Based on a 5 point satisfaction scale<p>
        </a>
        <script>
          function go(data) {
            if (data.Score > 0) {
              document.getElementById('stfcs-score-number').innerHTML = Math.round(data.Score * 100) / 100;
            }
          }
        </script>
        <script type='text/javascript' src='http://ssnip.satisfacts.com/WebServices/score.json.ashx?cb=go&pid=MTAzNw2'></script>
      </div>
    </div>
    <?php
}
